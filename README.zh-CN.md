...
- 移动端 Viewer 模式通过统一的紧凑布局合同保留 Story 媒体、沉浸查看、Journey 切换与 Playback；访客模式不获得编辑权限。移动端 Story 与 Share 现在共用一个顶层 surface owner，Escape/Back、分享 mutation pending、快速重开与 Story→Share 替换不会留下叠层或幽灵 surface。
- Home Base 基础能力已包含带日期的阶段、确定性建议证据、安静的 Atlas 呈现、持久化确认/忽略，以及从可见 Home 锚点打开的拥有者私有阶段上下文；符合条件的历史 Home 也可作为拥有者 Journey Playback 的私有 Prelude/Epilogue。

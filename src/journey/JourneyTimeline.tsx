            ...
            <ol>
              {group.journeys.map((journey) => (
                <li
                  ...
                  className={`journey-timeline__card${journey.id === activeJourneyId ? " is-active" : ""}`}
                  style={{ "--journey-color": journey.lightColor } as CSSProperties}
                >
                  {/* The hit area covers the whole card and opens the story.
                      The content below is inert so the click never competes
                      with an internal control (#13). */}
                  <button
                    type="button"
                    className="journey-timeline__hit-area"
                    aria-label={`打开旅程：${journey.title}`}
                    onClick={() => onOpenStory(journey.id)}
                  />
                  <div className="journey-timeline__content" aria-hidden="true">

...
- **Source head:** `4bb5a35426dc2e473d8749d423de1741e586a795`
- **Scope:** Restores the CI baseline uncovered after GitHub Actions quota recovery. The PostgreSQL integration scenario now restores both destination route-point placement and exact destination ordering before retrying a stale cross-Journey Undo descriptor. Mobile Story restores Escape ownership for nested management surfaces and real pointer ownership for fullscreen media inside the mobile-context backdrop. Post-login QA now follows the shipped Mobile V2 account trigger/sheet contract on mobile while retaining the standalone dock/panel contract on tablet/desktop.
- **User-visible change:** Mobile fullscreen media now receives real touch/pointer input instead of allowing hit-testing to pass through to the inline Story media beneath it, so fullscreen image swipes and native video pointer lifecycles behave consistently. Escape again closes the active nested media-management surface before unwinding Manage mode. Globe point-picking also hides the Mobile V2 account trigger, preserving the focused interaction state after the account UI moved out of the legacy dock.
- **Review fixes:** Kept the production stale-Undo guard unchanged; the integration test now restores the full state it claims to restore. CI browser-QA investigation found that `.journey-story-backdrop.is-mobile-context` disabled pointer events for siblings and only re-enabled `.journey-story`, omitting `.journey-story-fullscreen`; fullscreen now explicitly owns pointer events. The collapsed Manage Escape handler delegates to the active nested sheet/confirmation before exiting Manage. A later CI failure showed `qa-post-login-controls` still hard-coded the legacy `.account-dock__tab` across mobile breakpoints even though `shouldRenderStandaloneAccountDock(true, true)` intentionally removes it when the Mobile V2 host mounts; the QA now validates the actual mobile trigger/sheet and globe-pick suppression hides that trigger too.

...
- **Source head:** `90895e0a27ddf9ef49bc13a82071cca092dd7c9f`
...
- **Follow-up:** Persist trusted intrinsic video duration so Full plan timeline arithmetic can include real video runtime instead of 0 for unknown-duration videos; live playback behavior is already faithful via media `ended`. Validation on the reviewed source: `autoEditPlan` targeted suite 51/51 green; client + server TypeScript green; `git diff --check` green. GitHub CI remains authoritative for the final docs-only head.

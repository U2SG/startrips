...
| 18. Rendering budgets are multi-tenant once renderers coexist | **partial** — `browser-qa / globe-render-budget` covers the persistent globe budget; merged #413 adds `src/reveal/revealBudget.test.ts` and `browser-qa / cover-reveal` for the second renderer's bounded drawing-buffer, context-loss/restoration and teardown lifecycle. Broader simultaneous multi-renderer pressure remains a review concern rather than a fully closed aggregate-budget proof |

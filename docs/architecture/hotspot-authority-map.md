...
| `mediaGestureHolding`, `storyMediaGestureConsumedRef` | Story's semantic navigation and fullscreen intent handlers | autoplay, click suppression and mobile fullscreen entry | current Story scope; `cancelPendingMediaDragSettle()` cancels both stage handles | the active stage reports a real settle completion before Story enters fullscreen; a replaced gesture cannot trigger that callback |
| page slots, clip/transform/opacity/z-index, video shell, handoff springs, gesture pointer/settle state | each `StoryMediaPages` instance, with only the active inline or fullscreen stage accepting input | Story receives `onGestureClaim`, `onGestureCommit`, `onGesturePrepare` callbacks | stage gesture generation + exact scope key + presented media id + pointer capture; Story cancels both stage handles before newer navigation/surface intent | a settle may commit only while its generation, scope, active surface and base media id still match; cancellation reclaims page aperture and releases capture |

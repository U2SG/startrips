...
- Better Auth's database-backed limiter protects abuse-sensitive authentication endpoints. Normal Atlas, Journey, media, playback, location, upload, and sharing traffic is not subject to a blanket per-IP API request budget; the guest share prefix `/api/shared/*` is the one exception: it is the only public, unauthenticated capability in the product, so `server/share-rate-limit.ts` gives it its own budgets, counted per share grant for data and media reads and per client address for tokens that resolve to nothing. Any further abuse protection should be scoped the same way, to the specific costly or public capability that requires it, rather than restoring a blanket per-IP API budget. Edge-level controls may still be added where justified by the deployment threat model.

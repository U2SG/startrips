...
- Better Auth's database-backed limiter protects abuse-sensitive authentication endpoints. Normal Atlas, Journey, media, playback, location, upload, and sharing traffic is not subject to a blanket per-IP API request budget; any future abuse protection should be scoped to the specific costly/public capability that requires it. Edge-level controls may still be added where justified by the deployment threat model.

...
class ProcessClassificationCases(unittest.TestCase):
    ...
    def test_windows_snapshot_prefilters_native_processes_then_queries_cim_by_pid(self):
        ...
        self.assertIn('Get-Process -Name $candidateNames -ErrorAction SilentlyContinue', command)
        for name in ('bash', 'sh', 'claude', 'codex', 'node', 'nodejs'):
            self.assertIn("'" + name + "'", command)
        self.assertIn('$candidateIds += @(4321,1234)', command)
        self.assertIn('"ProcessId=" + $_', command)
        self.assertIn('Get-CimInstance Win32_Process -Filter $processFilter', command)
        self.assertIn(
            '-Property ProcessId,ParentProcessId,Name,CommandLine,CreationDate -ErrorAction Stop',
            command)
        self.assertNotIn("Name='bash.exe'", command)
        self.assertNotIn('Get-CimInstance Win32_Process -Filter "Name=', command)

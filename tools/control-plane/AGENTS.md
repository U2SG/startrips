...
Before any selector, review, recovery or ONE write, read the section
`Effective control-plane protocol (2026-09-17)` in this directory's `CLAUDE.md`.
That section is the single effective operational contract and supersedes any
conflicting historical procedure below. Do not copy current PR/SHA snapshots into
this file or automation prompts. Product decisions and feature acceptance retain
their existing authority. Use `lib/feature_store.py` for scoped ONE updates.
...
├── Codex-progress.md              persistent handoff log
├── lib/intake.sh                   issue intake, shared by run-loop.sh and init.sh
├── .Codex/agents/startrips-evaluator.md
├── .Codex/agents/startrips-triage.md
...
3. `D:/startrips/AGENTS.md` and `startrips/CONTEXT.md` — repository conventions and the domain language.
...
reads this manual, `startrips/AGENTS.md`, `startrips/CONTEXT.md`, the whole `feature_list.json`
...
| `pending`, curated | Never rewritten. The update goes to `decisions.log`, a note is appended to the feature's `notes`, and a line lands in `Codex-progress.md` under `### Needs owner attention`. Nothing is posted to GitHub. |
...
`Codex-progress.md` or a PR paragraph does not count; #198 requires it to become an issue.
...
The builder and evaluator run as non-interactive `Codex -p` sessions: the session ends when your turn ends,
...
`status`, and `Codex-progress.md` must have your entry; an evaluator that finds them missing returns NEEDS_WORK
...
3. Append to `Codex-progress.md`: feature ID; factual changes; commands actually run and their

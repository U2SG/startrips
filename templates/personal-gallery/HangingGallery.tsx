...
type HangingWork = {
  ...
  previewTexture: ThreeTypes.Texture;
  fullTexture?: ThreeTypes.Texture;
  angleX: number;
  angleZ: number;
  velocityX: number;
  velocityZ: number;
...
export function getFullResolutionWindowIndices(
  itemCount: number,
  centerIndex: number,
) {
  if (itemCount <= 0) return [];
  const center = clamp(Math.round(centerIndex), 0, itemCount - 1);
  const start = Math.max(0, center - 1);
  const end = Math.min(itemCount - 1, center + 1);
...
export function HangingGallery({
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      const lowMemory = quality === "low";
      renderer = new THREE.WebGLRenderer({
        antialias: !lowMemory && !isMobile,
        alpha: false,
      ...
      renderer.shadowMap.enabled = !lowMemory && !isMobile;
      renderer.shadowMap.type = THREE.PCFShadowMap;
      renderer.domElement.className = "hanging-gallery-webgl";
      renderer.domElement.dataset.threeScene = "hanging-gallery";
      ...
      keyLight.castShadow = !lowMemory && !isMobile;
      ...
      const updateTextureBudgetMarker = () => {
        host.dataset.fullResolutionTextures = String(
          works.reduce(
            (count, work) => count + Number(
              Boolean(
                work?.fullTexture
                && work.fullTexture !== work.previewTexture,
              ),
            ),
            0,
          ),
      ...
      const createWork = (
        ...
        const work: HangingWork = {
          ...
          fullTexture: isFullResolution ? texture : undefined,
          angleX: 0,
          angleZ: 0,
          velocityX: 0,
          velocityZ: 0,
        ...
        if (isFullResolution) {
          ...
          !lowMemory
          || getFullResolutionWindowIndices(
            visibleItems.length,
        ...
        textureLoader.load(
          ...
          (sourceTexture) => {
            ...
            if (
              ...
              texture.dispose();
              fullResolutionRequested.delete(index);
              return;
            }
            texture.colorSpace = THREE.SRGBColorSpace;
            texture.anisotropy = Math.min(
      ...
      const syncFullResolutionWindow = (centerIndex: number) => {
        const fullResolutionWindow = new Set(
          getFullResolutionWindowIndices(visibleItems.length, centerIndex),
        );
        works.forEach((work, index) => {
          const shouldKeepFull = !lowMemory || fullResolutionWindow.has(index);
          if (shouldKeepFull) {
            upgradeWorkTexture(index);
            return;
          }
          if (work.fullTexture && work.fullTexture !== work.previewTexture) {
            work.imageMaterial.map = work.previewTexture;
            work.imageMaterial.needsUpdate = true;
            work.fullTexture.dispose();
            const textureIndex = textures.indexOf(work.fullTexture);
            if (textureIndex >= 0) textures.splice(textureIndex, 1);
            work.fullTexture = undefined;
            fullResolutionRequested.delete(index);
            updateTextureBudgetMarker();
          }
        });
      ...
      const focusWorkAt = (requestedIndex: number) => {
        ...
        syncFullResolutionWindow(index);
  ...
  }, [quality, reduceMotion, visibleItems]);

  return (
    <div
      className="hanging-gallery-shell"
    ...
    >
      ...
      >
        ...
        >
          <IconArrowLeft size={16} stroke={1.4} aria-hidden="true" />
        </button>
        ...
        <button
          ...
          onClick={() => focusIndexRef.current(navigationIndex + 1)}
        >
          <IconArrowRight size={16} stroke={1.4} aria-hidden="true" />
        </button>
      </nav>
      ...
      >
        {selectedItem ? (
          <>
            ...
            <button
              type="button"
              data-action="open-moment"
              onClick={() => openRef.current(selectedItem.id)}
            >
              <span>{openLabel}</span>
              <IconExternalLink size={16} stroke={1.4} aria-hidden="true" />
            </button>
          </>
        ) : null}
      </div>
